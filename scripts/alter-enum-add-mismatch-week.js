...
(async () => {
  try {
    ...
    const beforeStr = before[0].COLUMN_TYPE;
    console.log('Valores actuales del ENUM:', (beforeStr.match(/'/g).length / 2));
    console.log('¿Ya tiene ESTIMATED_DATE_MISMATCH_WEEK?:', beforeStr.includes("'ESTIMATED_DATE_MISMATCH_WEEK'") ? 'SÍ' : 'NO');
    ...
    const afterStr = after[0].COLUMN_TYPE;
    console.log('¿ESTIMATED_DATE_MISMATCH_WEEK presente?:', afterStr.includes("'ESTIMATED_DATE_MISMATCH_WEEK'") ? '✅ SÍ' : '❌ NO');
    console.log('Total valores en ENUM:', (afterStr.match(/'/g).length / 2));
  } catch (err) {
